...
module.exports = {
	init(d, c, co, m, l, rich) {
		...
		setInterval(() => {
			database.getGames((games) => {
				games.forEach((g) => {
					if (g.phase !== 'Finished') {
						module.exports.httpGet(g.GameID, (response) => {
					...
					module.exports.httpGet(g.GameID, (response) => {
						...
						if (g.date !== date) {
							...
							for (let i = 0; i < members[0].length; i++) {
								...
								if (name === 'Anonymous') {
									database.getGameDataCountry(g.GameID, country, (Gdata) => {
										if (Gdata === undefined) {
									...
									database.getGameDataPlayer(g.GameID, name, (Gdata) => {
										if (Gdata === undefined) {
											database.addGameData(g.GameID, name, supplyCenters, units, country);
										}
										else if (Gdata.supplyCenters !== supplyCenters || Gdata.units !== units) {
